...
export const updateEmployeePersonalDetails = async (req, res) => {
  ...
  try {
    const [updatedRowsCount] = await personalDetailsSchema.update(
    ...
    return res.status(200).json({
      ...
      updatedDetails: updatedPersonalData,
    });
  } catch (error) {
...
export const getEmployeePersonalDataByEmployeeId = async (req, res) => {
  ...
  try {
    const personalData = await personalDetailsSchema.findOne({
      where: { employeeId: employeeId },
    });
    ...
    if (!personalData) {
      return res
        .status(404)
        .json({ message: "Personal data not found for the given employeeId" });
    }
...
export const getListOfDateOfBirth = async (startDate, endDate) => {
  try {
      ...
      where: {
        [Op.and]: [
          Sequelize.where(
            Sequelize.fn(
              "DATE_FORMAT",
              Sequelize.col("date_of_birth_epd"),
              "%m-%d"
            ),
            {
    ...
    for (const data of personalData) {
      ...
      const dateOfBirthWithCurrentYear = `${currentYear}-${data.dateOfBirth.substring(
        5
      )}`;
      // Push name with birthdate to the array
      namesWithBirthdate.push({
        name: name,
        date: dateOfBirthWithCurrentYear,
        type: "birthDay",
      });
    }
