...
export const addEmployeeEducationalDetail =  async (req, res) => {
    try {
      const { employeeId, degree, institute, startDate, endDate, percentage } = req.body;
  ...
  export const getEmployeeEducationalDetail = async (req, res) => {
    try {
        const educationalDetails = await EducationalSchema.find({ deleteFlag: false });
  ...
  export const putDeleteFlag = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Educational details deleted successfully" });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
}
