...
export default uploadDWRPath;

export const uploadDWRToFirebaseStorage = async (file) => {
    try {
        const storageRef = firebase.storage().ref();
        const fileExtension = path.extname(file.originalname);
        const uploadTask = storageRef.child(`dailyWorkReport/${Date.now()}${fileExtension}`).put(file.buffer, { contentType: file.mimetype });
        await uploadTask;
  ...
  export const readDWRFromFirebaseFile = async (firebaseFilePath) => {
    try {
